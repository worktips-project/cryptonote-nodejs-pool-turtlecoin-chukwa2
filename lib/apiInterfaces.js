...
function jsonHttpRequest(host, port, data, callback, path) {
    if (
        (config.coin == 'TurtleCoin' || (config.coin == 'NinjaCoin' && port == config.wallet.port))
                            ...
                            switch (jsonData.method) {
                                ...
                                case 'getBalance':
                                    if (replyJson.error != null) {
                                        ...
                                        switch (config.coin) {
                                            case 'NinjaCoin':
                                                tempReply.result.availableBalance = replyJson.unlocked / 100000;
                                                tempReply.result.lockedAmount = replyJson.locked / 100000;
                                                break;
                                            case 'TurtleCoin':
